...
DOC = Path("docs/stage72-production-backup-before-deploy-execution-result.md")
...
def main() -> int:
    ...
    if missing:
        print("Stage 72.36 production backup-before-deploy execution result guard failed. Missing markers:")
        for marker in missing:
            print(f" - {marker}")
        return 1

    print("Stage 72.36 production backup-before-deploy execution result guard: passed")
    return 0
