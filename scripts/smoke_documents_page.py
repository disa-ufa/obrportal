...
def main() -> None:
    require_contains(
        ...
        [
            ...
            "generation-events/${eventId}/download",
    ...
    require_contains(
        ...
        [
            ...
            "downloadAdminDocumentGenerationEvent(documentItem.id, event.id)",
            "document-generation-event-download-action",
            "Скачать версию",
            "getAdminDocumentGenerationEvents(documentItem.id, { limit: 20 })",
            "document-generation-events",
            "document-generation-events-load-action",
            ...
            "Аудит назначения",
            "Аудит организации",
            "document-attention-diagnostics-note",
            "Диагностика основана на статусе, файле, причине отзыва, назначении, организации и паспорте генерации PDF.",
            "Причина отзыва: не указана, заполните её для прозрачного аудита.",
            "Дата отзыва: не зафиксирована, проверьте историю аудита.",
            "Файл: черновик уже содержит файл, можно проверить и опубликовать.",
            "Файл: черновик пока без файла, загрузите PDF/скан перед публикацией.",
            "Паспорт генерации: нет даты генерации PDF, пересоберите документ.",
            "Паспорт генерации: версия шаблона не зафиксирована, пересоберите PDF.",
            "Организация: назначение без организации, PDF использует fallback-настройки.",
    ...
    require_contains(
        ...
        [
            "QR-код публичной проверки документа",
            "QR-код ведёт на публичную страницу проверки по номеру или коду документа",
            "Файл документа и личный кабинет не раскрываются",
            "Открыть публичную проверку",
            "Публичная проверка недоступна: документ отозван",
            "Публичная проверка появится после публикации",
            "QR-код и публичная ссылка скрыты, чтобы отозванный документ не использовали как действующий.",
