...
DOC = Path("docs/stage72-production-backup-before-deploy-execution-readiness-checkpoint-acceptance.md")
...
def main() -> int:
    ...
    if missing:
        print("Stage 72.34 production backup-before-deploy execution readiness checkpoint acceptance guard failed. Missing markers:")
        for marker in missing:
            print(f" - {marker}")
        return 1

    print("Stage 72.34 production backup-before-deploy execution readiness checkpoint acceptance guard: passed")
    return 0
