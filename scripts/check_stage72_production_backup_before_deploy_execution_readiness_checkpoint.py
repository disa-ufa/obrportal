...
DOC = Path("docs/stage72-production-backup-before-deploy-execution-readiness-checkpoint.md")
...
def main() -> int:
    ...
    if missing:
        print("Stage 72.32 production backup-before-deploy execution readiness checkpoint guard failed. Missing markers:")
        for marker in missing:
            print(f" - {marker}")
        return 1

    print("Stage 72.32 production backup-before-deploy execution readiness checkpoint guard: passed")
    return 0
