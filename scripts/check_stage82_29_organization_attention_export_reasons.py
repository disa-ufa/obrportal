from __future__ import annotations

from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PANEL = ROOT / "frontend" / "src" / "components" / "organization" / "OrganizationLearningOverviewPanel.jsx"
DOC = ROOT / "docs" / "stage82-organization-attention-export-reasons.md"


def fail(message: str) -> None:
    raise SystemExit(f"stage 82.29 organization attention export reasons guard failed: {message}")


def read_required(path: Path) -> str:
    if not path.exists():
        fail(f"{path.relative_to(ROOT)} is missing")
    return path.read_text(encoding="utf-8")


def main() -> None:
    panel = read_required(PANEL)
    doc = read_required(DOC)

    panel_markers = [
        "stage82_29_organization_attention_export_reasons",
        "ORGANIZATION_ATTENTION_EXPORT_REASON_LABELS",
        "reasonsColumn: \"Причины\"",
        "{ key: \"reasons\", label: ORGANIZATION_ATTENTION_EXPORT_REASON_LABELS.reasonsColumn }",
        "formatOrganizationAttentionReasonBadgesForExport",
        "buildOrganizationAttentionReasonBadges(enrollment)",
        ".join(\"; \")",
        "reasons: formatOrganizationAttentionReasonBadgesForExport(enrollment)",
        "data-stage-export-reasons",
    ]
    missing_panel = [marker for marker in panel_markers if marker not in panel]
    if missing_panel:
        fail("missing panel markers: " + ", ".join(missing_panel))

    doc_markers = [
        "Stage 82.29",
        "Frontend-only",
        "CSV column",
        "Причины",
        "semicolon-separated",
        "Backend unchanged",
        "Database unchanged",
        "No migration required",
    ]
    missing_doc = [marker for marker in doc_markers if marker not in doc]
    if missing_doc:
        fail("missing doc markers: " + ", ".join(missing_doc))

    print("stage 82.29 organization attention export reasons guard passed")


if __name__ == "__main__":
    main()
