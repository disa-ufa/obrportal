...
REQUIRED_STAGE77_2_CHANGED_FILES = {
...
def main() -> None:
    ...
    if manifest["current_stage"] != "77.2":
        fail("current_stage must be 77.2")
    ...
    if checkpoint.get("last_confirmed_stage") != "77.1":
        fail("last_confirmed_stage must be 77.1")
    if checkpoint.get("last_confirmed_head") != "19b8664":
        fail("last_confirmed_head must be 19b8664")
    ...
    for stage_id in ["74", "75", "75.1", "75.2", "76", "77.1", "77.2"]:
        if stage_id not in stages:
            fail(f"stage {stage_id} record is missing")

    stage77_1 = stages["77.1"]
    if stage77_1.get("status") != "production_deployed":
        fail("stage 77.1 status must be production_deployed")
    if stage77_1.get("head") != "19b8664":
        fail("stage 77.1 head must be 19b8664")
    if stage77_1.get("deployment_type") != "frontend-only":
        fail("stage 77.1 deployment_type must be frontend-only")
    if stage77_1.get("backend_runtime_changed") is not False:
        fail("stage 77.1 backend_runtime_changed must be false")
    if stage77_1.get("database_migration_run") is not False:
        fail("stage 77.1 database_migration_run must be false")

    stage77_2 = stages["77.2"]
    if stage77_2.get("status") != "implementation_ready":
        fail("stage 77.2 status must be implementation_ready")
    if stage77_2.get("branch") != "stage77-course-builder-card-ux":
        fail("stage 77.2 branch must be stage77-course-builder-card-ux")
    if stage77_2.get("base") != "develop":
        fail("stage 77.2 base must be develop")
    if stage77_2.get("deployment_type") != "frontend-only":
        fail("stage 77.2 deployment_type must be frontend-only")
    if stage77_2.get("backend_runtime_changed_expected") is not False:
        fail("stage 77.2 backend_runtime_changed_expected must be false")
    if stage77_2.get("database_migration_expected") is not False:
        fail("stage 77.2 database_migration_expected must be false")

    missing_stage77_checks = REQUIRED_STAGE77_2_CHECKS - set(stage77_2.get("required_checks", []))
    if missing_stage77_checks:
        fail(f"stage 77.2 missing required checks: {sorted(missing_stage77_checks)}")

    missing_changed_files = REQUIRED_STAGE77_2_CHANGED_FILES - set(stage77_2.get("changed_files", []))
    if missing_changed_files:
        fail(f"stage 77.2 missing changed files: {sorted(missing_changed_files)}")

    for doc in stage77_2.get("required_documents", []):
