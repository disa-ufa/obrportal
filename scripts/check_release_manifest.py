...
REQUIRED_STAGE77_CHANGED_FILES = {
...
def main() -> None:
    ...
    if manifest["current_stage"] != "77.1":
        fail("current_stage must be 77.1")
    ...
    if checkpoint.get("last_confirmed_stage") != "76":
        fail("last_confirmed_stage must be 76")
    if checkpoint.get("last_confirmed_head") != "d442540":
        fail("last_confirmed_head must be d442540")
    ...
    for stage_id in ["74", "75", "75.1", "75.2", "76", "77.1"]:
        if stage_id not in stages:
            fail(f"stage {stage_id} record is missing")

    stage76 = stages["76"]
    if stage76.get("status") != "production_deployed":
        fail("stage 76 status must be production_deployed")
    if stage76.get("branch") != "develop":
        fail("stage 76 branch must be develop")
    if stage76.get("head") != "d442540":
        fail("stage 76 head must be d442540")
    if stage76.get("deployment_type") != "frontend-only":
        fail("stage 76 deployment_type must be frontend-only")
    if stage76.get("backend_runtime_changed") is not False:
        fail("stage 76 backend_runtime_changed must be false")
    if stage76.get("database_migration_run") is not False:
        fail("stage 76 database_migration_run must be false")

    stage77 = stages["77.1"]
    if stage77.get("status") != "implementation_ready":
        fail("stage 77.1 status must be implementation_ready")
    if stage77.get("branch") != "stage77-course-builder-readiness":
        fail("stage 77.1 branch must be stage77-course-builder-readiness")
    if stage77.get("base") != "develop":
        fail("stage 77.1 base must be develop")
    if stage77.get("deployment_type") != "frontend-only":
        fail("stage 77.1 deployment_type must be frontend-only")
    if stage77.get("backend_runtime_changed_expected") is not False:
        fail("stage 77.1 backend_runtime_changed_expected must be false")
    if stage77.get("database_migration_expected") is not False:
        fail("stage 77.1 database_migration_expected must be false")

    missing_stage77_checks = REQUIRED_STAGE77_CHECKS - set(stage77.get("required_checks", []))
    if missing_stage77_checks:
        fail(f"stage 77.1 missing required checks: {sorted(missing_stage77_checks)}")

    missing_changed_files = REQUIRED_STAGE77_CHANGED_FILES - set(stage77.get("changed_files", []))
    if missing_changed_files:
        fail(f"stage 77.1 missing changed files: {sorted(missing_changed_files)}")

    for doc in stage77.get("required_documents", []):
