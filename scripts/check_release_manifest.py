...
REQUIRED_STAGE80_3_CHANGED_FILES = {
    "docs/release-manifest.json",
    "docs/stage80-learner-documents-backend-api-contract.md",
...
def main() -> None:
    ...
    if checkpoint.get("last_confirmed_stage") != "80.2":
        fail("last_confirmed_stage must be 80.2")
    if checkpoint.get("last_confirmed_head") != "10a3168":
        fail("last_confirmed_head must be 10a3168")
    ...
    for stage_id in ["77.7", "78.9", "79.6", "80.1", "80.2", "80.3"]:
    ...
    if stage80_3.get("branch") != "stage80-learner-documents-backend-api-contract":
        fail("stage 80.3 branch must be stage80-learner-documents-backend-api-contract")
    ...
    missing_checks = REQUIRED_STAGE80_3_CHECKS - set(stage80_3.get("required_checks", []))
    if missing_checks:
        fail(f"stage 80.3 missing required checks: {sorted(missing_checks)}")

    missing_files = REQUIRED_STAGE80_3_CHANGED_FILES - set(stage80_3.get("changed_files", []))
    if missing_files:
        fail(f"stage 80.3 missing changed files: {sorted(missing_files)}")
