...
def main() -> None:
    ...
    require_contains(
        ...
        [
            ...
            "err?.detail ?? err?.message ?? err?.response?.data?.detail ??",
            "export function isTechnicalApiErrorMessage(message)",
            "export function getSafeApiErrorMessage(",
            "isTechnicalApiErrorMessage(normalized)",
            "COMMON_API_ERROR_MESSAGES.technicalDetailsHidden",
            ...
            "COMMON_API_ERROR_MESSAGES.networkError",
            "status === \"401\"",
            "COMMON_API_ERROR_MESSAGES.notAuthenticated",
            "status === \"403\"",
            "COMMON_API_ERROR_MESSAGES.accessDenied",
            "status === \"404\"",
            "status === \"409\"",
            "status === \"422\"",
            "Number(status) >= 500",
            "COMMON_API_ERROR_MESSAGES.serverError",
