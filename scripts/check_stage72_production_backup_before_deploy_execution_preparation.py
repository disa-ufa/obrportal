...
DOC = Path("docs/stage72-production-backup-before-deploy-execution-preparation.md")
...
def main() -> int:
    ...
    if missing:
        print("Stage 72.25 production backup-before-deploy execution preparation guard failed. Missing markers:")
        for marker in missing:
            print(f" - {marker}")
        return 1

    print("Stage 72.25 production backup-before-deploy execution preparation guard: passed")
    return 0
