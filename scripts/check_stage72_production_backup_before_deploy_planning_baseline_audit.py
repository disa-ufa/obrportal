...
DOC = Path("docs/stage72-production-backup-before-deploy-planning-baseline-audit.md")
...
def main() -> int:
    ...
    if missing:
        print("Stage 72.23 production backup-before-deploy planning baseline audit guard failed. Missing markers:")
        for marker in missing:
            print(f" - {marker}")
        return 1

    print("Stage 72.23 production backup-before-deploy planning baseline audit guard: passed")
    return 0
