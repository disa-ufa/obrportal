...
## 34. Stage 15.21 admin courses page friendly errors - 2026-05-29

Goal: connect the strengthened shared safe error helpers to the admin courses page formatters.

Runtime change:
- updated `frontend/src/pages/AdminCoursesPage.jsx`;
- `formatCourseApiError`, `formatCourseModuleApiError` and `formatCourseLessonApiError` now use `getApiErrorStatus`;
- unknown course/module/lesson API messages now pass through `getSafeApiErrorMessage`;
- existing domain-specific Russian messages for courses, slugs, delete restrictions, modules, lessons and position conflicts are preserved;
- no backend API changes were required.
