...
def test_public_course_detail_exposes_outline_without_lesson_payloads() -> None:
    public_api = read(PUBLIC_API)
    public_schema = read(PUBLIC_SCHEMA)

    assert PUBLIC_COURSE_ENDPOINT_MARKER

    assert "from app.models.lesson_block import LessonBlock" not in public_api
    assert "PublicLessonBlockResponse" not in public_api
    assert "def build_public_lesson_block" not in public_api
    assert "content_json=block.content_json or {}" not in public_api
    assert "settings_json=block.settings_json or {}" not in public_api
    assert "blocks_by_lesson_id: dict[str, list[LessonBlock]]" not in public_api
    assert "LessonBlock.lesson_id.in_(lesson_ids)" not in public_api
    assert "LessonBlock.is_active.is_(True)" not in public_api
    assert "select(LessonBlock)" not in public_api
    assert "content_url=lesson.content_url" not in public_api
    assert "content_text=lesson.content_text" not in public_api

    assert "class PublicLessonBlockResponse" not in public_schema
    assert "content_json: dict[str, Any]" not in public_schema
    assert "settings_json: dict[str, Any]" not in public_schema
    assert "content_url: str | None" not in public_schema
    assert "content_text: str | None" not in public_schema
    assert "blocks: list[PublicLessonBlockResponse]" not in public_schema

    assert "class PublicCourseLessonResponse" in public_schema
    assert "content_type: str" in public_schema
    assert "position: int" in public_schema
    assert "is_required: bool" in public_schema
...
def test_account_course_detail_includes_active_lesson_blocks() -> None:
    ...
    assert 'if block.block_type == "quiz":' in account_api
    assert "sanitize_quiz_content_for_learner(" in account_api
    assert "content_json=content_json" in account_api
